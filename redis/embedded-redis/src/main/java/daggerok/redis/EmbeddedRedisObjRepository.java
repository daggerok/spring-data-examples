...
public interface EmbeddedRedisObjRepository extends PagingAndSortingRepository<EmbeddedRedisObj, Long> {

  /*
   * "LIKE (1): [IsLike, Like]is not supported for redis query derivation"
   */
  Stream<EmbeddedRedisObj> findAllByDataIsLike(@Param("data") final String data);

  /*
   * CONTAINING (1): [IsContaining, Containing, Contains]is not supported for redis query derivation
   */
  Stream<EmbeddedRedisObj> findAllByDataContaining(@Param("data") final String data);
}
